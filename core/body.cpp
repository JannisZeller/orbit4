...
class body {
   public:
    body::body(vec3D Pos, vec3D Vel, double m, double r, string str = "Default", string uSys = "SI", bool bool_massive = true, bool bool_movable = true) {
        name = str;
    ...
    void body::step(double dt, body* other, string algo = "rkf45a38") {
    ...
    void body::sys_step(double dt, string algo = "rkf45a38") {
        for (vector<body*>::iterator p = Bodies.begin(); p != Bodies.end(); ++p) {
            if ((*p)->movable) {
